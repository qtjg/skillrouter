...
- **Area dilution (PRD §4.4):** when `router.distinctiveness` is enabled and a scored candidate has a **neighbor** (registry overlay via `src/registry/neighbors.ts` — field-weighted overlap of id containment, name, triggers, technologies, intents, description) whose score is strictly higher, the weaker candidate pays up to 35% of its score, proportional to the overlay. Exact clones (overlay 1.0) lose the full 35%; a unique capability (overlay 0) pays nothing. The `neighbor` signal records the cut and appears in `skillrouter explain` rejections. Disabled with fewer than two candidates.
